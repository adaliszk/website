...
import { type PageFlagType, useFeatureFlagContext } from "./FeatureFlags";

export type MenuItem = {
    href: string;
    label: string;
    activePattern: RegExp;
    key: keyof PageFlagType;
};

export const MenuContext = createContextId<MenuItem[]>("io.adaliszk.www.menu");

export const MenuItemOrder: Array<keyof PageFlagType> = [
    //    "updates",
    "projects",
    // "snippets",
    // "tools",
...
export function useMenuContextProvider() {
    ...
    const menuItems: MenuItem[] = MenuItemOrder.map((flag) => ({
        enabled: featureFlags.pages[flag],
        flag,
    }))
        .map((feature) =>
